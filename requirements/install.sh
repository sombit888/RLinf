...
# Set install location to something user-writable
INSTALL_PREFIX="/scratch/$USER/installs"
...
if [[ " ${EMBODIED_TARGET[*]} " == *" $TARGET "* ]]; then
    ...
    git clone https://github.com/RLinf/LIBERO.git "$INSTALL_PREFIX/libero"
    echo "export PYTHONPATH=$INSTALL_PREFIX/libero:\$PYTHONPATH" >> .venv/bin/activate
fi
...
if [ "$TARGET" = "openvla" ]; then
    ...
    if [ "$ENABLE_BEHAVIOR" = "true" ]; then
        git clone -b RLinf/v3.7.1 --depth 1 https://github.com/RLinf/BEHAVIOR-1K.git "$INSTALL_PREFIX/BEHAVIOR-1K"
        cd "$INSTALL_PREFIX/BEHAVIOR-1K" && \
    ...
    git clone https://github.com/NVIDIA/Megatron-LM.git -b core_r0.13.0 "$INSTALL_PREFIX/Megatron-LM"

    if [ "$TEST_BUILD" != "true" ]; then
        APEX_CPP_EXT=1 APEX_CUDA_EXT=1 NVCC_APPEND_FLAGS="--threads 24" \
        APEX_PARALLEL_BUILD=24 uv pip install -r requirements/megatron.txt --no-build-isolation
    fi

    echo "export PYTHONPATH=$INSTALL_PREFIX/Megatron-LM:\$PYTHONPATH" >> .venv/bin/activate
